...
package com.ljx.view.calendar.view;
...
import com.ljx.view.calendar.announce.DayStatus;
import com.ljx.view.calendar.entity.ColorScheme;
import com.ljx.view.calendar.entity.DayEntity;
...
public final class DayView extends LinearLayout {
    ...
    private void initialize(Context context) {
        ...
        int padding = (int) (context.getResources().getDisplayMetrics().density * 3);
        ...
        int descPaddingTop = (int) (context.getResources().getDisplayMetrics().density * 5);
        tvDesc.setPadding(0, descPaddingTop, 0, 0);
    ...
    private void setBackgroundStatus(DayEntity entity, ColorScheme scheme) {
        switch (entity.status()) {
            ...
            case DayStatus.BOUND_M:
            case DayStatus.BOUND_R:
                int txWhite = Color.parseColor("#ffffff");
                tvDay.setTextColor(txWhite);
                tvDesc.setTextColor(txWhite);
                tvDesc.setText(entity.note());
                //setBackgroundColor(scheme.daySelectBackgroundColor());
                Drawable leftBg = selectedLeftRightBg();
                setBackground(leftBg);
                break;
            default:
                break;
        }
    ...
    private Drawable selectedLeftRightBg() {
        int blue = Color.parseColor("#3269F6");
        int radius = 12;
        GradientDrawable drawBg = new GradientDrawable();
        drawBg.setShape(GradientDrawable.RECTANGLE);
        drawBg.setCornerRadius(radius);
        drawBg.setColor(blue);
        drawBg.mutate();
        return drawBg.getConstantState().newDrawable();
    }
