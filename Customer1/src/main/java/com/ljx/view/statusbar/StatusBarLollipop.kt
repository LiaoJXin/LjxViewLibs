...
class StatusBarLollipop {
    ...
    companion object {
         ...
         */
        private fun startColorAnimation(startColor: Int, endColor: Int, duration: Long, windowWeakReference: WeakReference<Window>) {
            sAnimator?.cancel()
        ...
        fun setStatusBarColorForCollapsingToolbar(activity: Activity, appBarLayout: AppBarLayout,
                                                  collapsingToolbarLayout: CollapsingToolbarLayout, toolbar: Toolbar, statusColor: Int) {
            val window = activity.window
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS)
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)
            window.statusBarColor = Color.TRANSPARENT
                ...
                if (weakWindow != null) {
                    if (Math.abs(verticalOffset) > appBarLayout.height - collapsingToolbarLayout.scrimVisibleHeightTrigger) {
                        if (weakWindow.statusBarColor != statusColor) {
                            startColorAnimation(weakWindow.statusBarColor, statusColor,
                                collapsingToolbarLayout.scrimAnimationDuration, windowWeakReference)
                        }
                    } else {
                        if (weakWindow.statusBarColor != Color.TRANSPARENT) {
                            startColorAnimation(weakWindow.statusBarColor, Color.TRANSPARENT,
                                collapsingToolbarLayout.scrimAnimationDuration, windowWeakReference)
                        }
                    }
                }
